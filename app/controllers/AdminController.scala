...
class AdminController @Inject()(cc: ControllerComponents) extends AbstractController(cc) {
  ...
  def showedit(id:Long) = Action { implicit request: Request[AnyContent] =>
  ...
  def edit() = Action { implicit request: Request[AnyContent] =>
    ...
    val url = "http://localhost:9000/coupons/"+id
    val gson = new Gson
    val result = Http(url).put(gson.toJson(Coupon(title,desc,id.toLong,img,code))).header("content-type", "application/json").method("PUT").asString
    if (result.code == 200) Ok("")
    else NotFound("")
  }
  def create() = Action { implicit request: Request[AnyContent] =>
    ...
    val url = "http://localhost:9000/coupons"
    val gson = new Gson
    val result = Http(url).put(gson.toJson(Coupon(title,desc,id,img,code))).header("content-type", "application/json").method("POST").asString
    if (result.code == 200) Ok("")
    else NotFound("")
  }
}
